...
class multi {
	public static void main(String[] args) {
        ...
        n = s.nextInt();
        Academic_Performance[] a = new Academic_Performance[n];
        Sports_Performance[] p = new Sports_Performance[n];
        for(int i = 0; i < n; i++) {
            ...
            a[i] = new Academic_Performance(s1, s2, s3, s4, s5, degree);
            //a.display(name, phno);
            p[i] = new Sports_Performance(sportsName, sportScore);
            if(i == n-1)
                isLast = true;
            if(isLast == true){
                a[i].display(name, phno);
            }
        }
	}
}
